...
Still written by a later phase (slice 7b): the **normalized** people & artifact
nodes (link-derived), `symbol_events` (no current golden exercises it), and all
non-spine edges (`authored`/`reviewed`/`touches`/`owns`/…).
